...
# Get all images
for index1, public_prompt in enumerate(positive_prompts):
    for index2, personal_prompt in enumerate(positive_prompts):
        for common_step in range(0,28):
            image1 = pipe(prompt=public_prompt, num_inference_steps=total_step, guidance_scale=public_scale, common_step=common_step, prompt_unchanged=True, generator=generator, skip=True).images[0]
            image2 = pipe(prompt=personal_prompt, negative_prompt=negative_prompts[index2], num_inference_steps=total_step, guidance_scale=personal_scale, common_step=common_step, prompt_unchanged=False, generator=generator).images[0]
